...
export class ZrokHost {
  ...
  createTunnel(
    ...
    const starting: TunnelStatus = { state: 'starting' }
    if (user) starting.authUser = user
    if (pass) starting.authPass = pass
    ...
    child.stdout.on('data', (d) => {
      ...
      if (this.status.state !== 'running') {
        ...
        if (url) {
          ...
          if (user) running.authUser = user
          if (pass) running.authPass = pass
          this.status = running
        }
      }
    })
    ...
    child.stderr.on('data', (d) => {
      ...
      if (this.status.state !== 'running') {
        ...
        if (url) {
          const runningStderr: TunnelStatus = {
            state: 'running',
            reservedName: effectiveReservedName ?? undefined,
            url,
            startedAt,
          }
          if (user) runningStderr.authUser = user
          if (pass) runningStderr.authPass = pass
          this.status = runningStderr
        }
      }
    })
    child.on('error', (err) => {
