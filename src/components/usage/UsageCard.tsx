...
export function UsageCard(props: Props) {
  ...
  const [hoveredHeatmapDay, setHoveredHeatmapDay] =
    createSignal<HeatmapCell | null>(null);
  const [activityPreviewPosition, setActivityPreviewPosition] = createSignal({
    left: 0,
    top: 0,
  });
  const [activeTab, setActiveTab] = createSignal<
    "activity" | "models" | "providers"
  >("activity");
  ...
  const [usageSummary] = createResource(
    ...
    ({ workspacePath, days }) =>
      window.openpi.getUsageSummary({ workspacePath, days }),
  );

  const filteredDaily = createMemo(() => usageSummary()?.daily ?? []);
  const filteredModels = createMemo(() => usageSummary()?.models ?? []);
  const providerBuckets = createMemo(() =>
    aggregateProviders(filteredModels()),
  );
  ...
  const rangeCacheSavingsUsd = createMemo(() =>
    sumCacheSavingsForModels(filteredModels()),
  );
  ...
  const heatmapWeeks = createMemo(() =>
    buildHeatmapWeeks(filteredDaily(), heatmapWeekCount()),
  );
  ...
  const rangeDateSpan = createMemo(() =>
    formatRangeSpan(filteredDaily(), range()),
  );
  ...
  const scopeLabel = createMemo(
    () =>
      props.workspaceLabel?.trim() ||
      workspaceScopeFromPath(props.workspacePath),
  );
  ...
  const updateHeatmapPreview = (day: HeatmapCell, event: PointerEvent) => {
    ...
    setActivityPreviewPosition({
      left: fitsRight
        ? event.clientX + gap
        : Math.max(8, event.clientX - gap - width),
      ...
      <header class="usage-dashboard-hero">
        <div class="usage-dashboard-hero-copy">
          <div class="usage-dashboard-title-row">
            ...
            <span
              class="usage-scope-pill"
              title={props.workspacePath ?? "All indexed workspaces"}
            >
              {scopeLabel()}
            </span>
          </div>
          <Show when={needsProviderReindex()}>
            <p class="usage-reindex-hint" role="status">
              Provider labels are missing on older indexes. Open or refresh
              sessions in this project to re-index usage (v3).
            </p>
          </Show>
        </div>
        <div
          class="usage-range-tabs usage-range-tabs--hero"
          role="group"
          ...
          >
            <div class="usage-card-skeleton" aria-hidden="true">
              <div class="usage-metric-grid">
                <For each={[1, 2, 3, 4]}>
                  {() => <div class="usage-metric-panel-skeleton" />}
                </For>
              </div>
          ...
          <>
            <div class="usage-metric-grid">
              <article class="usage-metric-panel">
                <header class="usage-metric-panel-head">
                  ...
                  <p class="usage-section-desc">
                    Captured tokens in this range.
                  </p>
                </header>
                <p class="usage-metric-hero">
                  {formatTokenMetric(rangeTotals().totalTokens)}
                </p>
              ...
              <article class="usage-metric-panel">
                ...
                <p class="usage-metric-hero">
                  {rangeCacheHitRate() != null
                    ? formatCacheHitRate(rangeCacheHitRate())
                    : "—"}
                </p>
                <dl class="usage-metric-kv">
                  <div>
                ...
                <p class="usage-metric-foot usage-pricing-foot">
                  <Show
                    when={
                      rangeCacheSavingsUsd() != null &&
                      rangeCacheSavingsUsd()! > 0
                    }
                    fallback="Savings use pi-ai catalog rates when recognized."
                  >
                    Est. saved {formatCurrency(rangeCacheSavingsUsd()!)} from
                    cache.
                  </Show>
                </p>
              </article>
              ...
              <article class="usage-metric-panel">
                <header class="usage-metric-panel-head">
                  ...
                  <p class="usage-section-desc">
                    Reported session cost when providers expose it.
                  </p>
                </header>
                <p class="usage-metric-hero">
                  {rangeTotals().cost > 0
              ...
              <article class="usage-metric-panel">
                <header class="usage-metric-panel-head">
                  ...
                  <p class="usage-section-desc">
                    Average load per session in this range.
                  </p>
                </header>
                <p class="usage-metric-hero">
                  {tokensPerSession() != null
                    ? formatTokenMetric(tokensPerSession()!)
                    : "—"}
                </p>
                <dl class="usage-metric-kv">
                  <div>
              ...
              <Show when={activeTab() === "activity"}>
                <div class="usage-panel">
                  <header class="usage-panel-head">
                    ...
                    <p class="usage-section-desc">
                      Each square is one day of captured usage.
                    </p>
                  </header>
                  <div
                                ...
                                {(day) => (
                                  <button
                                    ...
                                    onPointerEnter={(event) =>
                                      updateHeatmapPreview(day, event)
                                    }
                                    onPointerMove={(event) =>
                                      updateHeatmapPreview(day, event)
                                    }
                      ...
                      {(preview) => (
                        ...
                        >
                          ...
                          <div class="usage-day-detail-stats">
                            <div>
                              <strong>
                                {formatTokenMetric(preview().totalTokens)}
                              </strong>
                              <span>tokens</span>
                            </div>
                            <div>
                              <strong>
                                {preview().turnCount.toLocaleString()}
                              </strong>
                              <span>turns</span>
                            </div>
                            <div>
                            ...
                            <Show when={preview().cacheHitRate != null}>
                              <div>
                                <strong>
                                  {formatCacheHitRate(preview().cacheHitRate)}
                                </strong>
                                <span>cache</span>
                              </div>
                            </Show>
                          ...
                          <div class="usage-day-models">
                            <span class="usage-day-models-title">
                              Top models
                            </span>
                            <Show
                                ...
                                <For each={preview().models.slice(0, 5)}>
                                  {(model, index) => (
                                    <li>
                                      ...
                                      <em>
                                        {formatTokenMetric(model.totalTokens)}
                                      </em>
                                    </li>
                                  )}
                                </For>
  ...
  return (
    <div class="usage-panel">
      ...
      <header class="usage-panel-head">
        ...
        <p class="usage-section-desc">
          Usage of models across your indexed sessions ·{" "}
          {formatTokenMetric(totalTokens())} captured
        </p>
      </header>
      ...
      <Show
        ...
        fallback={
          <div class="usage-models-placeholder">
            No model usage captured yet. Run a session and assistant turns will
            show up here.
          </div>
        }
      >
        <div class="usage-featured-grid">
          <For each={featured()}>
            {(model, index) => {
              const prev = previousByKey().get(
                modelKey(model.model, model.provider),
              );
              const delta = findDeltaPct(
                model.totalTokens,
                prev?.totalTokens ?? 0,
              );
              ...
              return (
                ...
                >
                  <span
                    class="usage-model-swatch"
                    style={{ background: color }}
                  />
                  <span class="usage-featured-rank">
                    {String(index() + 1).padStart(2, "0")}
                  </span>
                  <span class="usage-featured-name">
                    {formatModelName(model.model) || model.model || "unknown"}
                  </span>
                  <span class="usage-featured-volume">
                    {formatTokenMetric(model.totalTokens)}
                  </span>
                ...
                return (
                  ...
                  >
                    <span
                      ...
                      style={{ background: color }}
                    />
                    <span class="usage-rest-rank">
                      {String(index() + 4).padStart(2, "0")}
                    </span>
                    <span class="usage-rest-name">
                      {formatModelName(model.model) || model.model || "unknown"}
                    </span>
                    <span class="usage-rest-volume">
                      {formatTokenMetric(model.totalTokens)}
                    </span>
...
function LeaderboardRow(props: {
  ...
  const label = () =>
    formatModelName(props.model.model) || props.model.model || "unknown";
  const vendor = () => formatProviderLabel(props.model.provider);

  return (
    <li class="usage-leaderboard-row">
      <span class="usage-leaderboard-rank">
      ...
      <div class="usage-leaderboard-main">
        <div class="usage-leaderboard-top">
          ...
          <span class="usage-leaderboard-volume">
            {formatTokenMetric(props.model.totalTokens)}
          </span>
        </div>
        <div class="usage-leaderboard-sub">
          ...
          <span class="usage-leaderboard-share">
            {formatSharePct(props.sharePct)}
          </span>
...
function ProviderUsagePanel(props: {
  ...
  const [activeProviderKey, setActiveProviderKey] = createSignal<string | null>(
    null,
  );
  ...
  return (
    <div class="usage-panel">
      <header class="usage-panel-head">
        ...
        <p class="usage-section-desc">
          Token share by provider in this range ·{" "}
          {formatTokenMetric(totalTokens())} total
        </p>
      </header>
      <div class="usage-panel-subsection">
      ...
      <Show
        ...
        fallback={
          <div class="usage-models-placeholder">
            No provider breakdown yet. Assistant turns with model metadata will
            appear after sessions are indexed.
          </div>
        }
      >
          ...
          <For each={props.providers}>
            {(row, index) => {
              ...
              return (
                ...
                >
                  <span
                    class="usage-provider-swatch"
                    style={{ background: color }}
                  />
                  <span class="usage-provider-rank">
                    {String(index() + 1).padStart(2, "0")}
                  </span>
                  <span class="usage-provider-name">{row.model}</span>
                  <span class="usage-provider-volume">
                    {formatTokenMetric(row.totalTokens)}
                  </span>
                  <span class="usage-provider-share">
                    {formatSharePct(share)}
                  </span>
                  <Show when={(savings ?? 0) > 0}>
                    <span class="usage-provider-saving">
                      saved {formatCurrency(savings ?? 0)}
                    </span>
                  </Show>
                </button>
              );
...
function buildHeatmapWeeks(
  ...
  const start = addDays(
    today,
    -((weeksToShow - 1) * DAYS_PER_WEEK + today.getUTCDay()),
  );
