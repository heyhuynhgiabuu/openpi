...
test.describe('browser tunnel workspace picker', () => {
  ...
  test('shows BrowserWelcome with absolute path input on web (no native dialog)', async ({
    ...
    await page.route('**/api/ipc/**', async (route) => {
      ...
      if (channel.includes('get-app-info')) {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({ name: 'OpenPi', version: '0.2.7', releaseChannel: null }),
        })
        return
      }
      if (
        channel.includes('get-pref') ||
        channel.includes('get-customizations') ||
        channel.includes('get-workspace-summary') ||
        channel.includes('get-git-branch') ||
        channel.includes('tunnel')
      ) {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
    ...
    await expect(
      page.locator('#openpi-workspace-picker').getByPlaceholder('/home/andrea/Projects/openpi')
    ).toBeVisible()
  ...
  test('shows known projects list and allows picking', async ({ page }) => {
    ...
    await expect(
      page.locator('.homescreen-project.is-selected').filter({ hasText: 'openpi' })
    ).toBeVisible()
  })
  ...
  test('webBridge modal via pickWorkspace without payload', async ({ page }) => {
    ...
    await page.route('**/api/ipc/**', async (route) => {
      ...
      if (
        channel.includes('get-first-run') ||
        channel.includes('get-pref') ||
        channel.includes('get-customizations')
      ) {
        await route.fulfill({
      ...
      if (channel.includes('pick-workspace')) {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({ cancelled: false, path: '/home/andrea/Projects/openpi' }),
        })
        return
      }
      await route.fulfill({
