...
- The line counters live in `electron/git/gitDiffCount.ts`. Hunk-state tracking replaced prefix sniffing, so content that is exactly `-- ` or `++ ` counts, and lines outside a hunk do not. A combined hunk from an unmerged path counts its first column, which is stage 2 ("ours") and matches `git diff HEAD --numstat` for the same path; the tests assert that agreement across three conflict shapes rather than a hand-picked total.
- The renderer has its own prefix-sniffing counter: `countHunkLines` in `src/components/git/hunkUtils.ts` (used by `GitHunkActions`) skips any line starting with `---`, so on a patch that removes a markdown rule it reports 0/1 where `countDiffLines` reports 0/2 and the pane header agrees with git. Per-hunk counts can therefore disagree with the pane total.
