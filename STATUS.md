...
- `normalizeShortcut` in `src/lib/shortcutFormat.ts` has no callers, and its doc comment lists `⏎`/`⌫`/`⇥`/arrow mappings the code never implemented; the two `⌘\+`-style rules are unreachable because the earlier replace consumed the glyphs. Deleting it needs a decision, as does the stale `process.platform` fallback claim in the same file's header.
- `useSessionIndex.ts:69` builds a `collapsedGroups` set from `groupSessions(...)` that no component reads, so group collapsing has no effect anywhere. Removing the state needs a decision; nothing renders a workspace group label either, which is why the workspace branch of `groupSessions` is only exercised by tests.
...
- `formatTokens` in `src/lib/sessionView.ts` has no callers anywhere; the usage panel formats tokens with `formatTokenMetric` (`1.2M`) instead, and this one uses a different shape (`1.2k tok`). Deleting it needs a decision. Its rounding also has no upper guard, so 999,999 renders as `1000.0k tok`.
