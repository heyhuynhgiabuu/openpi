...
- Session IPC authorization and read-model handlers were extracted from `electron/session/ipc.ts` into `sessionAuth.ts` / `inspectionIpc.ts` / `lifecycleIpc.ts` (ipc.ts is now 230 lines). `electron/pi/sidecar.ts` (1056 → 183, split into context/env/slash-prompt/event-bridge/lifecycle plus four command-domain modules) and `src/hooks/useOpenPiSession.ts` (814 → 296, split into event pipe / actions / IPC wiring / task polling / task-card resolvers) are decomposed. A full scan now finds 30 files over the cap (~4,600 excess lines); the largest: `src/lib/ipc/_full.ts` 1565 (the IPC contract barrel — declared exempt: one purpose, declarative schemas, no logic), `UsageCard.tsx` 818 (now decomposed — see below), `App.tsx` 557, `gitDiffStatus.ts` 528, `ConversationWorkspace.tsx` 503 → 294 (split into ConversationPreviewSplit/useCoreSlashCommands/useTaskAgents), `sessionUsage.ts` 509, `FileTree.tsx` 504, `git/ipc.ts` 494, `keybindings.ts` 487, `Composer.tsx` 485. `UsageCard.tsx` is decomposed into usageRange/usageHeatmap/usageAggregates helpers and ProviderUsagePanel/UsageActivityPanel/UsageMetricPanels components (287 lines). `App.tsx` (557) is the remaining large root: its bulk is prop-threading to eight children (ConversationWorkspace alone takes ~50 props) — decomposing it well requires redesigning those child APIs first, which is its own planned slice rather than a mechanical split. The monotonic policy audit is recorded in `docs/audits/2026-09-15-monotonic-policy-audit.md`.
