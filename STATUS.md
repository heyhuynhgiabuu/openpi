...
- Session IPC authorization and read-model handlers were extracted from `electron/session/ipc.ts` into `sessionAuth.ts` / `inspectionIpc.ts` / `lifecycleIpc.ts` (ipc.ts is now 230 lines). A full scan still finds 32 files over the cap (~5,800 excess lines); the largest: `src/lib/ipc/_full.ts` 1565 (schema barrel, exempt candidate), `sidecar.ts` 1036, `UsageCard.tsx` 818, `useOpenPiSession.ts` 814, `App.tsx` 557, `gitDiffStatus.ts` 528, `ConversationWorkspace.tsx` 524, `sessionUsage.ts` 509, `FileTree.tsx` 504, `git/ipc.ts` 494, `keybindings.ts` 487, `Composer.tsx` 485. The monotonic policy audit is recorded in `docs/audits/2026-09-15-monotonic-policy-audit.md`.
