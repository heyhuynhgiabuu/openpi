...
- `getGitStatus` takes a file's line counts from the staged map whenever that file's index column is non-blank, so a file that is staged and then modified again (`MM`) shows only the staged delta; the working-tree delta is not added on top.
...
- `countDiffLines` counts patch lines by prefix, so content that is exactly `-- ` or `++ ` is still skipped as if it were a file header, and a combined diff from a merge conflict (two-column prefixes, `@@@` hunks) is counted naively. Tracking hunk state instead of sniffing prefixes would fix both; the counts are otherwise correct for every patch shape git produces, including whole-file additions and deletions, renames, binary files and `--unified=0`.
...
- `getGitFileDiff` repeats the same untracked-fallback block in its `unstaged` and `auto` branches, and `electron/git/gitDiffStatus.ts` is 542 lines against the 300-line rule.
